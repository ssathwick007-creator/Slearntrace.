...
const EditDistanceVis = () => {
    ...
    const [message, setMessage] = useState('Init first row/col: Distance from empty string.');
    ...
    const reset = () => {
        ...
        setCurrJ(1);
        setIsRunning(false);
        setMessage('Init first row/col: Distance from empty string.');
    };
    ...
    const nextStep = () => {
        ...
        if (s1[currI - 1] === s2[currJ - 1]) {
            nextDp[currI][currJ] = dp[currI - 1][currJ - 1];
            nextMsg = `Same character! ${s1[currI - 1]} === ${s2[currJ - 1]}. dp[${currI}][${currJ}] = dp[${currI - 1}][${currJ - 1}] = ${nextDp[currI][currJ]}.`;
        } else {
            let replace = dp[currI - 1][currJ - 1];
            let insert = dp[currI][currJ - 1];
            let remove = dp[currI - 1][currJ];
            nextDp[currI][currJ] = 1 + Math.min(replace, insert, remove);
            nextMsg = `Different! ${s1[currI - 1]} != ${s2[currJ - 1]}. dp[${currI}][${currJ}] = 1 + min(replace:${replace}, insert:${insert}, delete:${remove}) = ${nextDp[currI][currJ]}.`;
        }
        ...
        if (nextI > m) {
            setIsRunning(false);
            setMessage(`Finished! Edit Distance is ${nextDp[m][n]}.`);
        }
    };
            ...
            <div style={{ overflowX: 'auto' }}>
                <table style={{ borderCollapse: 'collapse', margin: '0 auto' }}>
                    <thead>
                        ...
                        <tr>
                            <th></th>
                            {Array.from({ length: n + 1 }).map((_, idx) => <th key={idx} style={{ padding: '8px', border: '1px solid #E2E8F0', background: '#F8FAFC' }}>{idx}</th>)}
                        </tr>
                    </thead>
                    <tbody>
                        {dp.map((row, i) => (
                            <tr key={i}>
                                ...
                                {row.map((val, j) => (
                                    <motion.td
                                        key={j}
                                        animate={{
                                            backgroundColor: currI === i && currJ === j ? '#FACC15' : (i > 0 && j > 0 ? (s1[i - 1] === s2[j - 1] ? '#DCFCE7' : '#EFF6FF') : '#F8FAFC'),
...
export default EditDistanceVis;
